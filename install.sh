...
if grep -q "CHROMEOS_RELEASE_NAME" /etc/lsb-release 2>/dev/null; then
	echo "ChromeOS is not supported! Use the chrome extension. https://chromewebstore.google.com/detail/narecord-web/mcambpfmpjnncfoodejdmehedbkjepmi"
	exit 1
fi
...
curl -sS https://github.com/Narehatechi/Narecord/releases/latest/download/NarelotlCli-Linux \
  --output "$outfile" \
  --location \
  --fail
