...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Narecord")
	var updateFlag = flag.Bool("repair", false, "Repair Narecord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Narecord")
	...
	if *versionFlag {
		fmt.Println("Narelotl Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Narecord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Narelotl' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Narecord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Narecord will likely not work.")
}
