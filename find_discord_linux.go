...
func init() {
	...
	if sudoUser != "" {
		if sudoUser == "root" {
			panic("Narelotl must not be run as the root user. Please rerun as normal user. Use sudo or doas to run as root.")
		}

		Log.Debug("Narelotl was run with root privileges, actual user is", sudoUser)
		...
		panic("Narelotl was run as root but neither SUDO_USER nor DOAS_USER are set. Please rerun me as a normal user, with sudo/doas, or manually set SUDO_USER to your username")
	}
	Home = os.Getenv("HOME")
